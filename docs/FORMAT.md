...
## Information Asymmetry and Provenance

When analyzing replay documents, be aware of the following asymmetries and differences between live arena matches and simulator-generated replays:

- **Live replay `a` reflects only established actions:** In official Screeps: Arena replays, `a` (actionLog) contains only actions that successfully resolved. Actions that failed due to lack of range, dead targets, or insufficient energy leave no trace in `a`.
- **Live replay `e.intent` is one-sided:** Telemetry recorded in `e.intent` originates from bot console output (`@intent ...`). Consequently, live matches capture intents only for instrumented bots on the user's own side; the opponent's console output is not accessible.
- **Simulator `a` is engine resolution:** In simulator-produced matches, `a` is emitted from the simulator's internal combat resolution stage. Whether this is strictly synonymous in every edge case with the official game engine's `actionLog` remains unverified.
- **`meta.arenaId` differs by producer:** live replays carry the MongoDB ObjectId of the arena (`6a86d8c454a3948a1e35f90c`), while a simulator has no such id and stores the arena name instead (`"Spawn Strike 3"`). Group by `meta.producer` before grouping by `arenaId`; never fabricate an id to make the two look alike.
- **Provenance via `meta.producer`:** Replay documents generated by the simulator specify `"screeps-arena-sim"` in `meta.producer`, distinguishing them from official live replays.
- **`e.intent` is optional:** `e.intent` is not a required replay field; uninstrumented matches and standard live replays will not have this extension.
